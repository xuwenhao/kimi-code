...
/**
 * Per-row holder annotation joined onto `GET /sessions` items (multi-instance,
 * design §3.8): `self` = this instance holds the write lease (the session is
 * materialized here), `peer` = another instance holds it (`address` is its
 * reachable base URL when the holder advertised one — the redirect target),
 * `none` = no lease on disk (the session is materialized nowhere). Purely
 * display/redirect metadata: the lease file stays the authority and is
 * re-checked on every materialization.
 */
