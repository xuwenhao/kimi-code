...
describe('AgentSwarm tool execution contract', () => {
  ...
  it('declares broad accesses and does not expose permission rule argument matching', async () => {
    ...
    expect(execution.toolData).toMatchObject({
      kind: 'agent_call',
  ...
  describe('registered user tools', () => {
    ...
    it('routes registered user tools through tool.call request/response', async () => {
      ...
      expect(
        ...
        [emit] permission.approval.resolved        { "sessionId": "test-session", "agentId": "main", "turnId": 0, "toolCallId": "call_lookup", "toolName": "Lookup", "action": "Approve Lookup", "approvalData": { "kind": "generic", "summary": "Approve Lookup", "detail": { "query": "moon" } }, "toolInput": { "query": "moon" }, "decision": "approved", "source": "auto" }
        [wire] permission.record_approval_result   { "turnId": 0, "toolCallId": "call_lookup", "toolName": "Lookup", "action": "Approve Lookup", "source": "auto", "result": { "decision": "approved" }, "time": "<time>" }
      ...
      expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
        [emit] tool.result                 { "turnId": 0, "toolCallId": "call_lookup", "output": "moon-result", "outcome": "completed" }
        [wire] context.append_loop_event   { "event": { "type": "tool.result", "parentUuid": "<uuid-3>", "toolCallId": "call_lookup", "result": { "output": "moon-result", "outcome": "completed" } }, "time": "<time>" }
