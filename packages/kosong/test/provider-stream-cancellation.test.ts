/**
 * Built-in provider stream cancellation scenarios — verifies that the public
 * `StreamedMessage.cancel()` contract reaches each SDK-owned transport, that
 * Google composes request/config signals, and that late native streams are
 * retired. Each SDK client is the only stubbed external boundary.
 *
 * Run: pnpm --filter @moonshot-ai/kosong exec vitest run test/provider-stream-cancellation.test.ts
 */
